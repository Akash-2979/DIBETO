import numpy as np
import pickle
import streamlit as st

# Load the model using forward slashes in the file path
loaded_model = pickle.load(open('C:/Users/prach/OneDrive/Desktop/Diabetes/trained_model.sav', 'rb'))

# Creating function for prediction
def diabetes_prediction(input_data):
    try:
        # Change the input_data to numpy array (ensure all inputs are floats)
        input_data_as_numpy_array = np.asarray(input_data, dtype=float)
        # Reshape the array as we are predicting for one instance
        input_data_reshaped = input_data_as_numpy_array.reshape(1, -1)
        prediction = loaded_model.predict(input_data_reshaped)
        if prediction[0] == 0:
            return 'The person is not diabetic'
        else:
            return 'The person is diabetic'
    except ValueError as e:
        return f"Error in input conversion: {e}"

def main():
    # Giving a title
    st.title('Diabetes Prediction')

    # Getting the input data from the user
    Pregnancies = st.text_input('Number of Pregnancies')
    Glucose = st.text_input('Glucose Level')
    BloodPressure = st.text_input('Blood Pressure value')
    SkinThickness = st.text_input('Skin Thickness value')
    Insulin = st.text_input('Insulin Level')
    BMI = st.text_input('BMI value')
    DiabetesPedigreeFunction = st.text_input('Diabetes Pedigree Function value')
    Age = st.text_input('Age of the Person')

    # Code for Prediction
    diagnosis = ''

    # Creating a button for Prediction
    if st.button('Diabetes Test Result'):
        if not all([Pregnancies, Glucose, BloodPressure, SkinThickness, Insulin, BMI, DiabetesPedigreeFunction, Age]):
            st.error("Please fill in all the fields before clicking the button.")
        else:
            try:
                # Convert input data to float and predict
                input_data = [float(Pregnancies), float(Glucose), float(BloodPressure), float(SkinThickness), float(Insulin), float(BMI), float(DiabetesPedigreeFunction), float(Age)]
                diagnosis = diabetes_prediction(input_data)
            except ValueError as e:
                st.error(f"Invalid input. Please enter numeric values only. Error: {e}")
    
    st.success(diagnosis)

if __name__ == '__main__':
    main()
